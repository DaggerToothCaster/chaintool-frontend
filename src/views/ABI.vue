  ...
  <div class="abi">
    ...
    <div class="scroll">
      <div class="container">
        ...
        <div class="contract-list">
          <div class="mobileContract" v-if="Object.keys(clickItem).length != 0">
            <div>
              <el-button
                size="mini"
                @click="(clickItem = []), styleHeight('init')"
                >{{ $t("abi.btnReturn") }}</el-button
              >
              <el-button
                size="mini"
                @click="(actionBar = !actionBar), styleHeight(500)"
                v-if="!actionBar"
                >{{ $t("abi.btnDevelop") }}</el-button
              >
              <el-button
                size="mini"
                @click="(actionBar = !actionBar), styleHeight(500)"
                v-if="actionBar"
                >{{ $t("abi.btnFold") }}</el-button
              >
            </div>
                ...
                <ul>
                  ...
                  <div effect="dark" placement="bottom">
                    <li @click="checkEtherscan">
                      <img
                        src="../assets/imgs/etherscanLogo.svg"
                        alt=""
                        class="etherscanLogo"
                      />
            ...
            <div class="contractInfo">
              <div>
                <span>{{ $t("abi.contractName") }}:</span>{{ clickItem.name }}
              </div>
              <div>
                <span>{{ $t("abi.blockchainAddress") }}:</span
                >{{ clickItem.network }}
              </div>
              <div>
                <span>{{ $t("abi.contractAddress") }}:</span
                >{{ clickItem.address }}
              </div>
            </div>
            <div class="mobileMainBox">
              <div v-for="(item, index) in tableData[0].ItemAbi" :key="index">
                ...
                >
                  ...
                  <span>{{ item.name }} </span>
                  <div>
                    <span>{{ $t("abi.returnContent") }}</span
                    ><span v-if="item.return"
                      ><json-viewer
                        :value="item.return.content"
                      ></json-viewer> </span
                    ><span v-if="!item.return"
              ...
              <div>
                <el-collapse
                  v-for="(item, index) in tableData[0].ItemAbi"
                  :key="index"
                  @change="styleHeight(400)"
                >
                  <el-collapse-item
                    v-if="
                      !(
                      ...
                      (item.stateMutability == 'Read' ||
                        item.stateMutability == 'Write') &&
                      item.name
                    "
                  >
                    <template slot="title">
                      <el-tag v-if="item.stateMutability == 'Read'">{{
                        item.stateMutability
                      }}</el-tag>
                      <el-tag
                        v-if="item.stateMutability == 'Write'"
                        type="warning"
                    ...
                    <div>
                      <div v-if="item.inputs && item.inputs.length > 0">
                        ...
                        <el-button type="primary" @click="submitAbiForm(item)"
                          >运行</el-button
                        >
                        <div v-if="item.return">
                          <span>{{ $t("abi.returnContent") }}</span>
                          <span
                            ><json-viewer
                              :value="item.return.content"
          ...
          <el-container class="main">
            <el-aside class="sidebar">
              <el-menu>
                <el-button class="btn" type="primary" @click="addContract()">{{
                  $t("abi.btnAddContract")
                }}</el-button>
                  ...
                  <div v-for="item in contractList" :key="item.network">
                    <el-submenu :index="item.network">
                      ...
                      <el-menu-item
                        ...
                        :key="key"
                        @click="openItem(data, item.network)"
            ...
            <el-container>
              <el-main>
                ...
                <el-table :data="tableData">
                  <el-table-column
                    prop="ItemName"
                    :label="$t('abi.contractName')"
                  >
                  </el-table-column>
                  <el-table-column
                    prop="ItemNetwork"
                    :label="$t('abi.blockchainAddress')"
                  >
                  </el-table-column>
                  <el-table-column
                    prop="ItemAddress"
                    :label="$t('abi.contractAddress')"
                  >
                  </el-table-column>
                </el-table>
                <div class="sol-body">
                    ...
                    <el-aside width="400px" class="column">
                      <el-menu :data="tableData">
                        <div v-if="tableData.length >= 1">
                          ...
                          >
                            <div
                              @click="transferParameters(item, index)"
                              class="contentList"
                              :key="index"
                            >
                              <el-submenu
                              ...
                              >
                                <template slot="title">
                                  ...
                                  <el-tag
                                    v-if="item.stateMutability == 'Payable'"
                                    type="danger"
                                  >
                                    Payable
                                  </el-tag>
                                  <el-tag
                                    v-if="!item.stateMutability"
                                    type="warning"
                                  >
                                    {{ item.stateMutability }}
                                  </el-tag>
                                  <span class="contentList-text">{{
                                    item.name
                                  }}</span>
                                </template>
                              </el-submenu>
                  ...
                  <div class="contentRight">
                    <div class="rightList">
                      <div v-if="parameter != null">
                        ...
                        <div
                          v-if="parameter[0].stateMutability == 'Payable'"
                          class="rightInput"
                        >
                          <div>
                            <span>value:</span>
                            <span>
                              <el-input
                                :placeholder="$t('abi.inputValue')"
                                v-model="overrides.value"
                        ...
                        <div v-if="abiCardData.length != 0">
                          ...
                          >
                            <div slot="header" class="clearfix">
                              <div>
                                <span v-if="item.typeFlag == 'error'">❌ </span>
                                {{ $t("abi.callFunctions") }}{{ item.function }}
                              </div>
                            </div>
                            <pre>
        ...
        <div class="dialog">
          <el-dialog
            :title="$t('abi.contract')"
            :visible.sync="dialogFormVisible"
            @close="closureInputBox"
          >
            <el-dialog
              width="45%"
              :title="$t('abi.commonABIs')"
            ...
            <el-form :model="form" :rules="rules" ref="form">
              ...
              >
                <el-input
                  v-model="form.name"
                  autocomplete="off"
                  :placeholder="$t('abi.validateName')"
                ></el-input>
              </el-form-item>
              <el-form-item
                :label="$t('abi.blockchainNetwork')"
                prop="network"
                :label-width="formLabelWidth"
                class="formLabelWidth"
              >
                <el-select
                  ...
                  :placeholder="$t('abi.selectNetwork')"
                >
                  <el-option
              ...
              <el-form-item
                ...
                prop="abi"
                :label-width="formLabelWidth"
                class="el-textarea formLabelWidth"
              >
                <textarea
                  autocomplete="off"
                  v-model="form.abi"
              ...
              <div class="popUpBox">
                <ul>
                  ...
                  <li class="upload-demo">
                    <el-button
                      @click="getABIFromEtherscan"
                      :loading="btnChangEnable"
                      :disabled="btnChangEnable"
                      >{{ $t("abi.etherscanGet") }}</el-button
                    >
                  </li>
                </ul>
              </div>
            ...
            <div slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="onSubmit('form')">
                {{ $t("pubilc.sure") }}</el-button
              >
            </div>
          </el-dialog>
          <el-dialog
            :title="$t('abi.lookOverABI')"
            :visible.sync="ABIVisible"
            width="30%"
            center
          >
            <div class="visible">
              <el-button round @click="checkJSONABI">
                {{ $t("abi.jsonABI") }}</el-button
              >
              <el-button round @click="checkHumanReadableABI"
                >{{ $t("abi.readableABI") }}
              </el-button>
            </div>
            <el-input
            ...
            <span slot="footer" class="dialog-footer">
              <el-button
                type="primary"
                @click="copy(checkABI, '复制成功', '.dialog-footer')"
                >{{ $t("pubilc.copy") }}</el-button
              >
              <el-button type="danger" @click="ABIVisible = false">{{
                $t("abi.quit")
              }}</el-button>
            </span>
          </el-dialog>
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.query.id == undefined || this.form.abi == "") {
      ...
      for (let i = 1; i < this.network.length; i++) {
        if (
          this.network[i].currencySymbol == this.$route.params.currencySymbol ||
  ...
  methods: {
    ...
    async dbGetABI(id) {
      try {
        await axios.get(intefUrl.getABI + "?id=" + id).then(async (res) => {
    ...
    async shareContract(Item) {
      ...
      for (let i in this.network) {
        if (
          this.network[i].networkName == Item.network ||
          this.network[i].chainID == Item.network
        ) {
          url =
    ...
    async getNameAndABI(network, address) {
      ...
      try {
        await axios.get(requestNetwork).then((res) => {
          abi = [JSON.stringify(res.data.name), JSON.stringify(res.data.abi)];
        });
        return abi;
      } catch (error) {
        return false;
    ...
    async getABIFromEtherscan() {
      ...
      let abi = await this.getNameAndABI(this.form.network, this.form.address);
    ...
    remove(name, target) {
      ...
      if (localData.length != 0 || localData != null) {
        for (let i = 0; i < localData.length; i++) {
          if (
            localData[i].address === target[0] &&
            localData[i].network === target[1]
          ) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          if (await this.validABI(this.form.abi)) {
            ...
            //打开页面
            await this.openItem(this.pageData, this.pageData.network);
          }
        } else {
          return false;
        }
    ...
    runReadFunction() {
      ...
      for (let i in this.tableData[0].ItemAbi) {
        if (
          this.tableData[0].ItemAbi[i].inputs &&
          this.tableData[0].ItemAbi[i].inputs.length == 0 &&
          this.tableData[0].ItemAbi[i].stateMutability == "Read"
        ) {
    ...
    async submitAbiForm(Item) {
      ...
      if (thisChainId != this.chainId) {
        await this.$confirm(
          this.$t("abi.switchNetworkPrompt") + this.clickItem.network + "?",
          ...
          .catch(() => {
            this.$message.error(this.$t("abi.connectionRequestError"));
            return;
          });
      } else {
      ...
      if (abiObj != null) {
        try {
          ...
          if (
            ...
            cardContent = await contract[Item.name](...args, overrides);
          } else {
            cardContent = await contract[Item.name](...args);
          }

          console.log("函数允信1个结果", cardContent,cardContent._isBigNumber);
          if (cardContent._isBigNumber) {
            cardContent={
				value:cardContent,
				format:cardContent*1,
			};
			if(cardContent.format>1e18){
				cardContent.fromWei=cardContent.format/1e18
			}
          }
		  console.log("函数允信2个结果", cardContent);
        ...
        setTimeout(() => {
          try {
            document.querySelector(".main").style.height =
              document.querySelector(".mobileContract").offsetHeight + "px";
          } catch (error) {}
        }, value);
      }
    },
